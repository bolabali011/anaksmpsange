...
async function processThumbnails() {
    ...
    const processingPromises = videosData.map(async (video) => {
        ...
        if (!thumbnailOptimizedSuccessfully && video.thumbnail.includes('postercdn.com')) {
            ...
            // Perhatikan bahwa di kode asli Anda, kedua kondisi if adalah '/snaps/'. Ini kemungkinan typo.
            // Saya asumsikan Anda ingin mencoba '/splash/' jika '/snaps/' gagal, atau sebaliknya.
            // Untuk contoh ini, saya akan mencoba salah satu alternatif jika yang pertama gagal.
